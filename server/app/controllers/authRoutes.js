...
//json secret key
var jsonSecret = "53%&WUEY&&^8IOWKJ8vv&^&QMJPZERSBV#4*";
...
var auth = function (req, res, next) {
    ...
    if (token) {
        ...
        jwt.verify(token, "53%&WUEY&&^8IOWKJ8vv&^&QMJPZERSBV#4*", function (err, decoded) {
            if (err) {
                return res.json({
                    success: false,
  ...
  async.waterfall([
    ...
    function(token, user, done) {
      var smtpTransport = nodemailer.createTransport({
        service: 'gmail', 
        auth: {
          user: 'skillmock@gmail.com',
          pass: 'skillmock@123'//process.env.GMAILPW
        },
      tls: {
